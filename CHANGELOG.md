...
* [BREAKING][behavior][rust] The built-in PSWAP observers are no longer registered automatically. Observer setup is now opt-in and composed on the builder: register observers via `ClientBuilder::with_note_observer` / `ClientBuilder::with_transaction_observer` (deduplicated by `name()`, so the same observer cannot fire twice). A plainly built client tracks nothing but the note screener; to keep PSWAP tracking, register `PswapChainObserver` and `PswapTransactionObserver` explicitly. Observers can only be registered at build time (the runtime `Client::with_transaction_observer` is removed). `StateSync::new` now takes the full `Vec<Arc<dyn OnNoteReceived>>` (screener first) instead of a single screener, and `OnNoteReceived` gains a defaulted `on_sync_start()` hook (so observers reused across syncs can reset per-sync state) and a `Send + Sync` bound ([#2280](https://github.com/0xMiden/rust-sdk/issues/2280)).
